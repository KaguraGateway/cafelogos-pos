...
final class CartTests: XCTestCase {
    ...
    func testCoffee() throws {
        ...
        let cart = Cart(items: [
            ...
            // ハレノヒ・サイフォン
            try CartItem(coffee: coffee1, brew: siphon, quantity: 1),
            // ハレノヒ・ペーパー
            try CartItem(coffee: coffee1, brew: paper, quantity: 1),
            // アイス・アイス
            try CartItem(coffee: coffee2, brew: ice, quantity: 5),
        ])
        // お値段の計算ができるかどうか
        ...
        XCTAssertThrowsError(try CartItem(coffee: coffee1, brew: ice, quantity: 1))
    ...
    func testOtherProduct() throws {
        let cart = Cart(items: [
            CartItem(product: pan, quantity: 50)
        ])
        // お値段の計算ができるかどうか
        ...
        // 豆の数量不足で購入不可扱いになるはず
        XCTAssertFalse(CartItem(product: pan, quantity: 101).canBuy())
    }
    ...
    func testChangeCart() throws {
        ...
        cart.setItemQuantity(itemIndex: 0, newQuantity: 2)
        XCTAssertEqual(cart.totalQuantity, 2)
        // 同じ商品、同じ淹れ方の商品を追加してもカート内の配列数は変わらない
        cart.addItem(newItem: try CartItem(coffee: coffee1, brew: flannnel, quantity: 2))
        XCTAssertEqual(cart.getItems().count, 1)
        XCTAssertEqual(cart.totalQuantity, 4) // 注文品数だけが増えるはず
        // 同じ商品、違う淹れ方の商品を追加するとカート内の配列数が増える
        cart.addItem(newItem: try CartItem(coffee: coffee1, brew: paper, quantity: 1))
        XCTAssertEqual(cart.getItems().count, 2)
        XCTAssertEqual(cart.totalQuantity, 5)
        // 違う商品を追加するとカート内の配列数が増える
        cart.addItem(newItem: try CartItem(coffee: coffee2, brew: flannnel, quantity: 1))
