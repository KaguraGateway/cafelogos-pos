...
func toPayment(dao: PaymentDao) -> Payment {
    return Payment(id: dao.id, type: PaymentType.cash, orderIds: [], paymentAmount: UInt64(dao.paymentAmount.doubleValue), receiveAmount: UInt64(dao.receiveAmount.doubleValue), paymentAt: dao.paymentAt, updatedAt: dao.updatedAt, syncAt: dao.syncAt, callNumbers: Array(dao.callNumbers))
}
...
public struct PaymentRealm: PaymentRepository {
    ...
    func save(payment: Payment) {
        let dao = PaymentDao(value: ["id": payment.id, "paymentType": PaymentTypeEnumDao.CASH, "receiveAmount": payment.receiveAmount, "paymentAmount": payment.paymentAmount, "changeAmount": payment.changeAmount, "paymentAt": payment.paymentAt, "updatedAt": Date(), "syncAt": payment.syncAt as Any, "callNumbers": payment.callNumbers])
